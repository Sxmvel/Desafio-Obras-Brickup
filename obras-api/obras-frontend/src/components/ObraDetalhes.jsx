...
const ObraDetalhes = () => {
    ...
    const handleSubmitEtapa = (e) => {
        ...
        dispatch(addEtapa(etapaComObraId)).then(() => {
            dispatch(fetchObraDetalhada(id)); // atualiza etapas
            dispatch(fetchProgressoObra(id)); // atualiza progresso
            setNovaEtapa({ nome: '', descricao: '', status: 'PENDENTE' }); // limpa form
        });
    };
            ...
            {obra.etapas && obra.etapas.length > 0 ? (
                <ul>
                    {obra.etapas.map((etapa) => (
                        <li key={etapa.id}>
                            <strong>{etapa.nome}</strong> - {etapa.status}
                        </li>
                    ))}
                </ul>
            ) : (
